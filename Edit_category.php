<?php
    ...
    // Kiểm tra tham số ID truyền trên URL
    $id = $_GET['id'] ?? null;
    if (!$id) {
    ...
    $currentCategory = $categoryController->getCategoryById($id);
    if (!$currentCategory) {
        header("Location: Admin_category.php");
        exit();
    }
    ...
    // Xử lý khi Admin nhấn cập nhật form chỉnh sửa
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (!empty($data['name'])) {
            if ($categoryController->updateCategory($id, $data)) {
                header("Location: Admin_category.php");
                exit();
            } else {
                $message = "Đã xảy ra lỗi hệ thống, không thể cập nhật dữ liệu.";
                $statusType = "error";
            }
        } else {
            $message = "Tên danh mục không được phép bỏ trống!";
            $statusType = "warning";
        }
        
        // Giữ lại chữ vừa nhập trên form nếu có lỗi
        $currentCategory = array_merge($currentCategory, $data);
    }
?>
<!DOCTYPE html>
<html lang="vi">
<head>
  ...
  <title>Chỉnh sửa danh mục — Đời sống sức khoẻ</title>
...
<body>
  <div class="app">
    ...
    <div class="main">
      ...
      <main class="content">
        <div class="page-head">
          <h1>Chỉnh sửa danh mục mã số #<?php echo (int)$id; ?></h1>
        </div>
        ...
        <section class="card" style="padding: 30px; max-width: 700px; margin: 0 auto;">
          ...
          <form method="POST" action="Edit_category.php?id=<?php echo (int)$id; ?>">
            <!-- Chỉ hiển thị ô nhập Tên danh mục phù hợp với Database -->
            <div class="form-group">
              <label for="name">Tên danh mục <span style="color: red;">*</span></label>
              <input type="text" id="name" name="name" class="form-control" value="<?php echo htmlspecialchars($currentCategory['name'] ?? ''); ?>" required>
            </div>

            <div class="btn-group">
              <button type="submit" class="btn btn-primary"><i class="icon-save"></i> Cập nhật thay đổi</button>
              <a href="Admin_category.php" class="btn" style="background: #e2e8f0; color: #4a5568; text-decoration: none; padding: 10px 20px; border-radius: 6px; font-size: 14px;">Hủy bỏ</a>
            </div>
          </form>
        </section>
      </main>
